...
router.use('', async function(req, res, next) {
    ...
    let book = await db.query(`SELECT * FROM book WHERE isJin = 1 and id >=
	((SELECT MAX(id) FROM book where isJin = 1)-(SELECT MIN(id) FROM book where isJin = 1)) * RAND() + (SELECT MIN(id) FROM book where isJin = 1)  LIMIT 10;
`);
